-- MySQL Script generated by MySQL Workbench
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema TP_Final_grupo_03
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema TP_Final_grupo_03
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `TP_Final_grupo_03` ;
USE `TP_Final_grupo_03` ;

-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`usuario`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`usuario` (
  `id_usuario` INT NOT NULL AUTO_INCREMENT,
  `email` VARCHAR(50) NOT NULL,
  `password` VARCHAR(45) NOT NULL,
  `nombre` VARCHAR(45) NOT NULL,
  `apellido` VARCHAR(45) NOT NULL,
  `dni` VARCHAR(45) NOT NULL,
  `birthdate` DATE NOT NULL,
  `fecha_alta` DATE NOT NULL,
  `activado` BIT NOT NULL,
  PRIMARY KEY (`id_usuario`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC) VISIBLE,
  UNIQUE INDEX `dni_UNIQUE` (`dni` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`rol`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`rol` (
  `id_rol` INT NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(45) NOT NULL,
  `descripcion` VARCHAR(100) NULL,
  PRIMARY KEY (`id_rol`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`usuario_rol`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`usuario_rol` (
  `id_usuario` INT NOT NULL,
  `id_rol` INT NOT NULL,
  PRIMARY KEY (`id_usuario`, `id_rol`),
  INDEX `rol_FK_idx` (`id_rol` ASC) VISIBLE,
  CONSTRAINT `usuario_FK`
    FOREIGN KEY (`id_usuario`)
    REFERENCES `TP_Final_grupo_03`.`usuario` (`id_usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `rol_FK`
    FOREIGN KEY (`id_rol`)
    REFERENCES `TP_Final_grupo_03`.`rol` (`id_rol`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`tipo_licencia`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`tipo_licencia` (
  `id_tipo_licencia` INT NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(45) NOT NULL,
  `descripcion` VARCHAR(100) NULL,
  PRIMARY KEY (`id_tipo_licencia`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`chofer`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`chofer` (
  `id_chofer` INT NOT NULL AUTO_INCREMENT,
  `numero_licencia` VARCHAR(45) NOT NULL,
  `id_tipo_licencia` INT NOT NULL,
  PRIMARY KEY (`id_chofer`),
  UNIQUE INDEX `license_number_UNIQUE` (`numero_licencia` ASC) VISIBLE,
  UNIQUE INDEX `id_tipo_licencia_UNIQUE` (`id_tipo_licencia` ASC) VISIBLE,
  CONSTRAINT `tipo_licencia_FK`
    FOREIGN KEY (`id_tipo_licencia`)
    REFERENCES `TP_Final_grupo_03`.`tipo_licencia` (`id_tipo_licencia`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = binary;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`marca`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`marca` (
  `id_marca` INT NOT NULL,
  `nombre` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id_marca`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`modelo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`modelo` (
  `id_modelo` INT NOT NULL,
  `nombre` VARCHAR(45) NOT NULL,
  `id_marca` INT NOT NULL,
  PRIMARY KEY (`id_modelo`),
  UNIQUE INDEX `id_marca_UNIQUE` (`id_marca` ASC) VISIBLE,
  CONSTRAINT `marca_FK`
    FOREIGN KEY (`id_marca`)
    REFERENCES `TP_Final_grupo_03`.`marca` (`id_marca`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`unidad_de_transporte`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`unidad_de_transporte` (
  `id_unidad_de_transporte` INT NOT NULL AUTO_INCREMENT,
  `patente` VARCHAR(45) NOT NULL,
  `posicion_actual` VARCHAR(150) NOT NULL,
  `anio_fabricacion` TINYINT NOT NULL,
  `numero_chasis` VARCHAR(45) NOT NULL,
  `id_marca` INT NOT NULL,
  `id_modelo` INT NOT NULL,
  PRIMARY KEY (`id_unidad_de_transporte`),
  UNIQUE INDEX `patente_UNIQUE` (`patente` ASC) VISIBLE,
  UNIQUE INDEX `numero_chasis_UNIQUE` (`numero_chasis` ASC) VISIBLE,
  UNIQUE INDEX `id_marca_UNIQUE` (`id_marca` ASC) VISIBLE,
  UNIQUE INDEX `id_modelo_UNIQUE` (`id_modelo` ASC) VISIBLE,
  CONSTRAINT `marca_FK`
    FOREIGN KEY (`id_marca`)
    REFERENCES `TP_Final_grupo_03`.`marca` (`id_marca`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `modelo_FK`
    FOREIGN KEY (`id_modelo`)
    REFERENCES `TP_Final_grupo_03`.`modelo` (`id_modelo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`service`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`service` (
  `id_service` INT NOT NULL AUTO_INCREMENT,
  `fecha_service` DATE NOT NULL,
  `detalle` VARCHAR(150) NOT NULL,
  `costo` DECIMAL(10,2) UNSIGNED NOT NULL,
  `kilometraje_actual_unidad` INT UNSIGNED NOT NULL,
  `interno` BIT NOT NULL,
  `id_usuario` INT NOT NULL,
  `id_vehiculo` INT NOT NULL,
  PRIMARY KEY (`id_service`),
  UNIQUE INDEX `id_usuario_UNIQUE` (`id_usuario` ASC) VISIBLE,
  UNIQUE INDEX `id_vehiculo_UNIQUE` (`id_vehiculo` ASC) VISIBLE,
  CONSTRAINT `usuario_FK`
    FOREIGN KEY (`id_usuario`)
    REFERENCES `TP_Final_grupo_03`.`usuario` (`id_usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `vehiculo_FK`
    FOREIGN KEY (`id_vehiculo`)
    REFERENCES `TP_Final_grupo_03`.`unidad_de_transporte` (`id_unidad_de_transporte`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`tipo_remolque`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`tipo_remolque` (
  `id_tipo_remolque` INT NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(45) NOT NULL,
  `peso_maximo` INT NOT NULL,
  PRIMARY KEY (`id_tipo_remolque`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`remolque`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`remolque` (
  `id_remolque` INT NOT NULL AUTO_INCREMENT,
  `id_tipo_remolque` INT NOT NULL,
  PRIMARY KEY (`id_remolque`),
  UNIQUE INDEX `id_tipo_remolque_UNIQUE` (`id_tipo_remolque` ASC) VISIBLE,
  CONSTRAINT `unidad_de_transporte_FK`
    FOREIGN KEY (`id_remolque`)
    REFERENCES `TP_Final_grupo_03`.`unidad_de_transporte` (`id_unidad_de_transporte`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `tipo_remolque_FK`
    FOREIGN KEY (`id_tipo_remolque`)
    REFERENCES `TP_Final_grupo_03`.`tipo_remolque` (`id_tipo_remolque`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`tipo_vehiculo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`tipo_vehiculo` (
  `id_tipo_vehiculo` INT NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id_tipo_vehiculo`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`viaje`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`viaje` (
  `id_viaje` INT NOT NULL AUTO_INCREMENT,
  `consumo_combustible_previsto` DECIMAL(10,2) NOT NULL,
  `consumo_combustible_real` DECIMAL(10,2) ZEROFILL NOT NULL,
  `kilometros_previstos` DECIMAL(10,2) NOT NULL,
  `kilometros_reales` DECIMAL(10,2) ZEROFILL NOT NULL,
  `origen` VARCHAR(150) NOT NULL,
  `destino` VARCHAR(150) NOT NULL,
  `fecha_salida` DATETIME NOT NULL,
  `fecha_llegada_estimada` DATETIME NOT NULL,
  `fecha_llegada` DATETIME NULL,
  PRIMARY KEY (`id_viaje`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`tipo_carga`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`tipo_carga` (
  `id_tipo_carga` INT NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(45) NOT NULL,
  `descripcion` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`id_tipo_carga`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`carga`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`carga` (
  `id_carga` INT NOT NULL AUTO_INCREMENT,
  `peso` DECIMAL(10,2) ZEROFILL NOT NULL,
  `fragil` BIT NOT NULL,
  `id_tipo_carga` INT NOT NULL,
  `id_viaje` INT NOT NULL,
  PRIMARY KEY (`id_carga`),
  UNIQUE INDEX `id_tipo_carga_UNIQUE` (`id_tipo_carga` ASC) VISIBLE,
  UNIQUE INDEX `id_viaje_UNIQUE` (`id_viaje` ASC) VISIBLE,
  CONSTRAINT `viaje_FK`
    FOREIGN KEY (`id_viaje`)
    REFERENCES `TP_Final_grupo_03`.`viaje` (`id_viaje`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `tipo_carga_FK`
    FOREIGN KEY (`id_tipo_carga`)
    REFERENCES `TP_Final_grupo_03`.`tipo_carga` (`id_tipo_carga`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`desvio`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`desvio` (
  `id_desvio` INT NOT NULL AUTO_INCREMENT,
  `razon` VARCHAR(100) NOT NULL,
  `tiempo` TIME NOT NULL,
  PRIMARY KEY (`id_desvio`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`viaje_desvio`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`viaje_desvio` (
  `id_viaje` INT NOT NULL,
  `id_desvio` INT NOT NULL,
  PRIMARY KEY (`id_viaje`, `id_desvio`),
  INDEX `desvio_FK_idx` (`id_desvio` ASC) VISIBLE,
  CONSTRAINT `viaje_FK`
    FOREIGN KEY (`id_viaje`)
    REFERENCES `TP_Final_grupo_03`.`viaje` (`id_viaje`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `desvio_FK`
    FOREIGN KEY (`id_desvio`)
    REFERENCES `TP_Final_grupo_03`.`desvio` (`id_desvio`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`viaje_chofer`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`viaje_chofer` (
  `id_viaje` INT NOT NULL,
  `id_chofer` INT NOT NULL,
  PRIMARY KEY (`id_viaje`, `id_chofer`),
  INDEX `chofer_FK_idx` (`id_chofer` ASC) VISIBLE,
  CONSTRAINT `viaje_FK`
    FOREIGN KEY (`id_viaje`)
    REFERENCES `TP_Final_grupo_03`.`viaje` (`id_viaje`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `chofer_FK`
    FOREIGN KEY (`id_chofer`)
    REFERENCES `TP_Final_grupo_03`.`chofer` (`id_chofer`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`cliente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`cliente` (
  `id_cliente` INT NOT NULL AUTO_INCREMENT,
  `email` VARCHAR(50) NOT NULL,
  `dni` VARCHAR(45) NOT NULL,
  `telefono` VARCHAR(45) NOT NULL,
  `nombre` VARCHAR(45) NOT NULL,
  `apellido` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id_cliente`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC) VISIBLE,
  UNIQUE INDEX `dni_UNIQUE` (`dni` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`factura`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`factura` (
  `id_factura` INT NOT NULL AUTO_INCREMENT,
  `numero_factura` VARCHAR(45) NOT NULL,
  `fecha_facturacion` DATE NOT NULL,
  `fecha_pago` DATE NULL,
  `id_cliente` INT NOT NULL,
  `id_viaje` INT NOT NULL,
  PRIMARY KEY (`id_factura`),
  UNIQUE INDEX `id_cliente_UNIQUE` (`id_cliente` ASC) VISIBLE,
  UNIQUE INDEX `id_viaje_UNIQUE` (`id_viaje` ASC) VISIBLE,
  CONSTRAINT `cliente_FK`
    FOREIGN KEY (`id_cliente`)
    REFERENCES `TP_Final_grupo_03`.`cliente` (`id_cliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `viaje_FK`
    FOREIGN KEY (`id_viaje`)
    REFERENCES `TP_Final_grupo_03`.`viaje` (`id_viaje`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`viaje_unidad_de_transporte`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`viaje_unidad_de_transporte` (
  `id_viaje` INT NOT NULL,
  `id_unidad_de_transporte` INT NOT NULL,
  PRIMARY KEY (`id_viaje`, `id_unidad_de_transporte`),
  INDEX `id_unidad_de_transporte_idx` (`id_unidad_de_transporte` ASC) VISIBLE,
  CONSTRAINT `viaje_FK`
    FOREIGN KEY (`id_viaje`)
    REFERENCES `TP_Final_grupo_03`.`viaje` (`id_viaje`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `unidad_de_transporte_FK`
    FOREIGN KEY (`id_unidad_de_transporte`)
    REFERENCES `TP_Final_grupo_03`.`unidad_de_transporte` (`id_unidad_de_transporte`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`vehiculo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`vehiculo` (
  `id_vehiculo` INT NOT NULL,
  `numero_motor` VARCHAR(45) NOT NULL,
  `kilometraje` DECIMAL(10,2) NOT NULL,
  `id_tipo_vehiculo` INT NOT NULL,
  UNIQUE INDEX `numero_motor_UNIQUE` (`numero_motor` ASC) VISIBLE,
  UNIQUE INDEX `id_tipo_vehiculo_UNIQUE` (`id_tipo_vehiculo` ASC) VISIBLE,
  INDEX `unidad_de_transporte_FK_idx` (`id_vehiculo` ASC) VISIBLE,
  CONSTRAINT `unidad_de_transporte_FK`
    FOREIGN KEY (`id_vehiculo`)
    REFERENCES `TP_Final_grupo_03`.`unidad_de_transporte` (`id_unidad_de_transporte`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `tipo_vehiculo_FK`
    FOREIGN KEY (`id_tipo_vehiculo`)
    REFERENCES `TP_Final_grupo_03`.`tipo_vehiculo` (`id_tipo_vehiculo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `TP_Final_grupo_03`.`costo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TP_Final_grupo_03`.`costo` (
  `id_costo` INT NOT NULL AUTO_INCREMENT,
  `detalle` VARCHAR(45) NOT NULL,
  `importe` DECIMAL(10,2) NOT NULL,
  `id_factura` INT NOT NULL,
  PRIMARY KEY (`id_costo`),
  UNIQUE INDEX `id_factura_UNIQUE` (`id_factura` ASC) VISIBLE,
  CONSTRAINT `factura_FK`
    FOREIGN KEY (`id_factura`)
    REFERENCES `TP_Final_grupo_03`.`factura` (`id_factura`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
